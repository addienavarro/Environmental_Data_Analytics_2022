...
```
## What are GLMs? 
Generalized linear models -> simplest form is a simple linear regression ("lm")

## Hypothesis testing 
Stating your question in a way that stats can answer - Null hypothesis = "there is no relationship between these two variables" 

## linear model - two continuous variables
## Anova - does the variance change if we look at two categorical variables 
...
H0: The difference beween the sample mean and the value        is zero

Ha: The difference is NOT zero (two-sided)
    The difference is GREATER THAN zero (one-sided)
    The difference is LESS THAN zero (one-sides)
...
#Exercise 2:
#1.
ozone.plot.density <- ggplot(EPAair, aes(x=Ozone, fill=as.factor(Year)))+
  geom_density(alpha = 0.5)+
  labs(fill = "year")+
  title("Density Plot: Ozone By Year", position = "top")+
  xlab("Ozone AQI Value")+
  ylab("Density")

print(ozone.plot.density)

#2. 
ozone.boxplot <- ggplot(EPAair, aes(x=as.factor(Year), y=Ozone))+
  geom_boxplot()+
  stat_summary(geom="text", fun.y = max, vjust = -1, size = 3, label = c("a", "b"), color = "red")+
...
ggplot(EPAair, aes(x=as.factor(Year), y = Ozone))+
  ...
  stat_summary(geom="text", fun.y = max, vjust = -1, size = 4, label = c("b", "a"), color="red")+
  labs(x = "", y = "Ozone AQI Value")+
  ylim(0,150)
```
...
PM2.5_regression <- lm(data = EPAair, PM2.5 ~ Ozone)
...
library(corrplot)
cor_object <- cor(EPAair %>% select(SITE_LATITUDE:PM2.5)%>% drop_na())

corrplot(cor_object, method = "ellipse")
corrplot.mixed(cor_object, upper = "ellipse")
