...
e <- c("red", "white", "red", NA) # character vector, need to put n/a so that the data frame has 4 vectors
f <- c(TRUE, TRUE, TRUE, FALSE) # logical vector
dataframe1 <- data.frame(d,e,f) #
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) #tidy data is when rows are observations and columns are properties of that observation
...
```{r}
hightemps<-c(58,41,42,45,35,42,48,51,55,66)
hightemps
lowtemps<-c(30,24,19,25,21,18,22,24,29,43)
lowtemps
```
...
```{r}
tempdataframe.df<-data.frame(hightemps, lowtemps,celsius(hightemps), celsius(lowtemps))
names(tempdataframe.df)<-c("F Highs", "F Lows", "C Highs", "C Lows")
```
...
date_obj_today <- ymd(str_today)
date_obj_today
#you can mess with the ymd to change to dmy or mdy
